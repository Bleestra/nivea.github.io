...
Модель умеет «говорить»: каждый бит сэмплируется из её собственной вероятности (с температурой),
веса заморожены, рабочая память включена. Сравнение — 10 затравок по 200 байт из тестового текста,
по 800 сгенерированных байт (`gen_bench.py`, примеры — в [`samples.md`](samples.md)):

| Модель (T=0.8) | Настоящих слов | Дословно скопировано из обучения | Повторы (4-граммы) | Скорость |
|---|---|---|---|---|
| **SynapseBrain-LM** | **96.8%** | 2.6% | **0.0%** | **~72 000 байт/с** |
| Трансформер ×7.3 времени | 92.5% | 5.0% | 2.7% | 228 байт/с* |
| Трансформер того же времени | 85.5% | 0.0% | 0.0% | 582 байт/с* |

\* трансформер пересчитывает окно на каждом шаге; с KV-кэшем было бы ~770 байт/с
(1.3 мс/токен, см. выше) — мозг всё равно ~100× быстрее. Скорость мозга — при T=1.0
(при первом прогоне T=0.8 в замер попала прогревочная компиляция).

**Честно о качестве:** мозг пишет почти только настоящие слова и почти ничего не копирует, но
фразы у него — «салат» из правдоподобных кусков без смысла, и он хуже держит связь с затравкой
(например, не закрыл начатую ссылку `[[James Gregory…|Gr`). Трансформер ×7.3 лучше продолжает
тему и синтаксис, зато чаще зацикливается и копирует. Вывод: преимущество мозга в bpc во многом
идёт от памяти (совпадения, длинные контексты, адаптация), а трансформер лучше выучивает
обобщаемую структуру. Следующий шаг — популяции «темы» и «позиции в предложении» (семантическая
и синтаксическая память).

## Как качество растёт с данными

Обе модели учатся на первых 1–90 МБ и тестируются на одном и том же отложенном 1 МБ
(`scaling_bench.py`). Трансформер (2 слоя × 256, dropout 0.1) получает щедрый бюджет, валидацию
каждые 30 с и сохранение лучшей точки — переобучение ему не мешает.

| Данные | Мозг: время | Мозг: учится | Мозг: рабочая память | Трансформер: время | эпох | Трансформер |
|---|---|---|---|---|---|---|
| 1 МБ | 22 с | **1.657** | **1.880** | 480 с | 19.3 | 2.347 |
| 3 МБ | 29 с | **1.592** | **1.731** | 600 с | 8.1 | 2.097 |
| 10 МБ | 60 с | **1.536** | **1.644** | 720 с | 2.8 | 2.002 |
| 30 МБ | 154 с | **1.494** | **1.588** | 900 с | 1.2 | 1.930 |
| 90 МБ | 412 с | **1.456** | **1.521** | 1200 с | 0.5 | 1.823 |

- На любом объёме данных мозг лучше, тратя в 3–20 раз меньше времени (и одно ядро против четырёх).
  Особенно велик разрыв на малых данных: на 1 МБ мозг почти так же хорош, как трансформер на 90 МБ.
- ~20 с у мозга на малых объёмах — это фиксированная цена первого касания 5.9 ГБ памяти.
- Кривая трансформера круче, но она смешивает данные и вычисления: только на 1 МБ он дошёл до
  лучшей точки раньше конца бюджета; на больших объёмах он ограничен временем, а не данными.
  Сойдутся ли кривые на гораздо больших данных и вычислениях — этот эксперимент не показывает.
